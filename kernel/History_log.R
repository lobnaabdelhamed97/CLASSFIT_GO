...
getActionLog <- function(data) {
  tryCatch({
    if (purrr::is_empty(data['org_id']) == FALSE & purrr::is_empty(data['type']) == FALSE) {
          ...
          switch(paste(var),
                 ...
                 "4" = {
                   ...
                   if (length(payment_data) != 0) {
                     ...
                     addedAsClient <- FALSE
                     if (!is.null(i[["payType"]]) &&
                       (stringr::str_detect(i[["payType"]], "stripe") == TRUE || stringr::str_detect(i[["payType"]], "Stripe") == TRUE) &&
                       i[["subsID"]] <= 0 &&
                       i[["cost"]] > 0 &&
                       i[["gmRefundPlicyID"]] != 2) {
                   ...
                   if (i[["subsID"]] > 0 &&
                     length(RefundStatus) != 0 &&
                     RefundStatus[[1]]$gm_ply_refunded == 1 &&
                     length(payment_data) != 0 &&
                     i[["gmRefundPlicyID"]] != 2) {
                     i[["payType"]] <- ''
                   ...
                   if (length(RefundStatus) != 0 &&
                     RefundStatus[[1]]$gm_ply_refunded == 0 &&
                     i[["gmRefundPlicyID"]] != 2) {
                     i[["payType"]] <- " "
                 ...
                 "12" = {
                   payment_data <- payment(i[["usrID"]], i[["gm_id"]], i[["actionCreateDate"]])
                   if (stringr::str_detect(i[["payType"]], "stripe") == TRUE &&
                     i[["subsID"]] <= 0 &&
                     i[["cost"]] > 0) {
                     i[["notes"]] <- 'User has been refunded via stripe as per refund policy'
                   ...
                   if (is.null(i[["coupon"]]) &
                     i[["cost"]] == 0 &
                     i[["subsID"]] <= 0 &
                     stringr::str_detect(i[["notes"]], "No refund is due as class is expired.") == FALSE) {
                     i[["notes"]] <- "No refund is due as no payment was made for this class."
                   }
                   if (stringr::str_detect(i[["classIsFree"]], "y") == TRUE || stringr::str_detect(i[["classPayType"]], "onsite") == TRUE) {
                     i[["appendDetail"]] <- 'false'
            ...
            ,
                 ...
                 "5" =
                 {
                   if (is.null(i[["usrID"]]) == FALSE &&
                     is.null(i[["subsID"]]) == FALSE &&
                     is.null(i[["actionCreateDate"]]) == FALSE) {
                   ...
                   if (!is.null(i[["subsType"]])) {
                     ...
                     if (i[["notes"]] == "Client purchased this package from iframe price" &&
                       i[["subsID"]] > 0 &&
                       i[["cost"]] > 0) {
                       i[["notes"]] <- ''
                     }
      ...
      if (length(data) == 0 &&
        purrr::is_empty(orgData[[1]]$accCreatedAt) == FALSE &&
        searchArr[[2]] >= 1 &&
        purrr::is_empty(searchArr[[1]]) == FALSE &&
        (searchArr[[1]] == 2 || searchArr[[1]] == 1)) {
        # appened date with dict of the log
...
refund_status <- function(guests, gm_type, currency_symbol = "", actionCreateDate) {
  ...
  if (length(coupon_list) > 0) {
    foreach(i = coupon_list, j = guests, k = refunded) %do% {
      if ('refunded_note' %in% j &&
        j[['refunded_note']] == '' &&
        i[['ply_id']] == j[['gm_ply_ply_id']] &&
        i[['gm_id']] == j[['gm_ply_gm_id']] &&
        i[['discount_value']] == 'true' &&
        k == 1) {
  ...
  if (length(refund) > 0) {
    foreach(i = refund, j = guests) %do% {
      if ('refunded_note' %in% j &&
        j[['refunded_note']] == '' &&
        i[['player_id']] == j[['gm_ply_ply_id']] &&
        i[['game_id']] == j[['gm_ply_gm_id']] &&
        i[['refund_amount']] != '' &&
        i[['refund_amount']] > 0) {
        j[['refunded_note']] <- paste0("Refunded ", currency_symbol, " ", i[['refund_amount']], "")
      }
      else if ('refunded_note' %in% j &&
        j[['refunded_note']] == '' &&
        i[['player_id']] == j[['gm_ply_ply_id']] &&
        i[['game_id']] == j[['gm_ply_gm_id']] &&
        i[['refund_amount']] != '' &&
  ...
  foreach(i = refunded, j = guests) %do% {
    if ('refunded_note' %in% j &&
      ...
      exists(gm_type) &&
      gm_type == 'y' &&
      i == 0) { j[['refunded_note']] <- paste0("No refund") }
  }
  return(guests)
}
